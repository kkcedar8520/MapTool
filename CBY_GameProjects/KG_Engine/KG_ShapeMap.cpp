...
HRESULT KG_ShapeMap::CreateVertexData()
{
	HRESULT hr = S_OK;
	m_VerTex.clear();
...
HRESULT KG_ShapeMap::CreateIndexData()
{
	HRESULT hr = S_OK;
...
void KG_ShapeMap::UpdateVertexData(std::vector<PNCT_VERTEX>& list)
{
	int size = list.size();
	m_VerTex.clear();
...
void KG_ShapeMap::SetMatrix(D3DXMATRIX* world, D3DXMATRIX* view, D3DXMATRIX* proj)
{
	KG_Model::SetMatrix(world, view, proj);
}

KG_ShapeMap::KG_ShapeMap()
{
}
